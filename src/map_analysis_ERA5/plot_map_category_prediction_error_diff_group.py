...
parser.add_argument('--months', type=int, nargs="+", help='Months to be processed.', required=True)
...
parser.add_argument('--category', type=str, help='The category used. If not set then assume there is no categories', default=None)
...
if args.category is None:
    ...
    category = []
    _raw_category = args.category.split(",")

    for i, c in enumerate(_raw_category):
        if c != "":
            c = int(c)
            category.append(c)
    
    category_str = ",".join(["%d" % c for c in category])
...
for model_version in args.model_versions:
    ...
    # Step 1: merge selected categories
    total_Emean =  ds[ds_varnames["Emean"]].weighted(ds["total_cnt"]).mean(dim="category")
    total_E2mean = ds[ds_varnames["E2mean"]].weighted(ds["total_cnt"]).mean(dim="category")
    total_cnt    = ds["total_cnt"].sum(dim="category")

    # Step 2: compuate through years
    total_Emean  = total_Emean.weighted(total_cnt).mean(dim="start_ym").rename("total_Emean")
    total_E2mean = total_E2mean.weighted(total_cnt).mean(dim="start_ym").rename("total_E2mean")
    ...
    total_cnt = total_cnt.sum(dim="start_ym").rename("total_cnt")
...
for j in range(ds_ref.dims["latitude"]):
    for i in range(ds_ref.dims["longitude"]):
        _tmp = scipy.stats.ttest_ind_from_stats(
            ...
            nobs1 = npdata[0]["cnt"][j, i],
            mean2 = npdata[1]["mean"][j, i],
            std2  = npdata[1]["std"][j, i],
            nobs2 = npdata[1]["cnt"][j, i],
            equal_var = False,
...
fig.suptitle("[%s minus %s] %04d-%04d month=%s, lead_pentad=%d, category=%s" % (

...
for year in list(range(args.year_rng[0], args.year_rng[1]+1)):
    ...
    filenames.append(
        os.path.join(args.input_dir, "ECCC-S2S_{model_version:s}_{varset:s}::{varname:s}_{yyyymm:s}.nc".format(
...
fig.suptitle("[%s] %04d-%04d month=%d, lead_pentad=%d" % (args.model_version, args.year_rng[0], args.year_rng[1], args.month, args.lead_pentad), size=20)
...
_shading = new_ds["total_Emean"].to_numpy() / plot_info["factor"]
...
_contour = new_ds["total_Estderr"] / plot_info["factor"]
...
_significant = np.abs(new_ds["total_Emean"].to_numpy()) - new_ds["total_Estderr"].to_numpy()
